...
const SustainableFashion = () => {
  const [susfashion, setSusFashion] = useState([]);
  const [search, setSearch] = useState("");
  const navigate = useNavigate();

  useEffect(() => {
    axios
      .get("http://localhost:5000/sustainable-fashion")
      .then((res) => setSusFashion(res.data))
      .catch((error) => {
  ...
  const handleViewDetails = (id) => {
    navigate(`/sustainable-fashion/${id}`);
  };

  const filteredItems = susfashion.filter((item) =>
