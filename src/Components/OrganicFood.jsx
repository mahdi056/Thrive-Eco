...
const OrganicFood = () => {
  const [organicFood, setOrganicFood] = useState([]);
  const [search, setSearch] = useState("");
  const navigate = useNavigate();

  useEffect(() => {
    axios
      .get("http://localhost:5000/organic-food")
      .then((res) => {
        setOrganicFood(res.data);
      })
      .catch((error) => {
        console.error("Error fetching organic food data", error);
      });
  }, []);

  const handleViewDetails = (id) => {
    navigate(`/organic-food/${id}`);
  };

  const filteredItems = organicFood.filter((item) =>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="mb-6">
        <input
          type="text"
          placeholder="Search organic food..."
          value={search}
